...
def build_env_name(task, shared_memory, use_image_obs):
    ...
    del task
    env_name = "BlockPushMultimodalFixed"
...
class BlockPushMultimodalFixed(block_pushing.BlockPush):
    """2 blocks, 2 targets."""
    ...
    def __init__(
        ...
        super(BlockPushMultimodalFixed, self).__init__(
...
class BlockPushHorizontalMultimodal(BlockPushMultimodalFixed):
    def _reset_object_poses(self, workspace_center_x, workspace_center_y):
        # Reset block poses.
...
if "BlockPushMultimodalFixed-v0" in registration.registry.env_specs:
    del registration.registry.env_specs["BlockPushMultimodalFixed-v0"]

registration.register(
    id="BlockPushMultimodalFixed-v0", entry_point=BlockPushMultimodalFixed, max_episode_steps=350
)

registration.register(
    id="BlockPushMultimodalFixedFlipped-v0",
    entry_point=BlockPushHorizontalMultimodal,
    max_episode_steps=25,
)

registration.register(
    id="SharedBlockPushMultimodalFixed-v0",
    entry_point=BlockPushMultimodalFixed,
    kwargs=dict(shared_memory=True),
    max_episode_steps=350,
)
registration.register(
    id="BlockPushMultimodalFixedRgb-v0",
    entry_point=BlockPushMultimodalFixed,
    max_episode_steps=350,
